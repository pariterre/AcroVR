...
public class ControlLeftArmAbduction : MonoBehaviour
{
...
//        else
        ...
        MainParameters.Instance.joints.nodes[5].Q[node] = (float)dof;
        ToolBox.GetInstance().GetManager<GameManager>().InterpolationDDL();
        ToolBox.GetInstance().GetManager<GameManager>().DisplayDDL(5, true);

        ToolBox.GetInstance().GetManager<DrawManager>().ControlOneFrame();
        //        ToolBox.GetInstance().GetManager<DrawManager>().ControlLeftArmAbduction((float)dof);
    }
}
