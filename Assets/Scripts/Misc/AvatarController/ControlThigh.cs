...
public class ControlThigh : MonoBehaviour
{
...
//        else
        ...
        MainParameters.Instance.joints.nodes[0].Q[node] = (float)dof;
        ToolBox.GetInstance().GetManager<GameManager>().InterpolationDDL();
        ToolBox.GetInstance().GetManager<GameManager>().DisplayDDL(0, true);

        ToolBox.GetInstance().GetManager<DrawManager>().ControlThigh((float)dof);
    }
}
