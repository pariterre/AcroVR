...
public class ControlRightArmFlexion : MonoBehaviour
{
...
//        else
        ...
        MainParameters.Instance.joints.nodes[2].Q[node] = (float)dof;
        ToolBox.GetInstance().GetManager<GameManager>().InterpolationDDL();
        ToolBox.GetInstance().GetManager<GameManager>().DisplayDDL(2, true);

        ToolBox.GetInstance().GetManager<DrawManager>().ControlOneFrame();
        //        ToolBox.GetInstance().GetManager<DrawManager>().ControlRightArmFlexion((float)dof);
    }
}
